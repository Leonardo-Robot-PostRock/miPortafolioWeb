...
export const Skills = ({ darkMode }) => {
  ...
  const skillsTitleClasses = `z-30 mb-96 flex cursor-pointer items-center rounded-lg p-3 text-center font-Raleway text-4xl 
  ...
  const skillsSectionClasses = `absolute mx-auto min-w-280 md:mt-28 ${
    count === 1
      ? 'hidden'
      : count === 0
  ...
  const skillsContentClasses = `mx-2 mt-10 rounded-lg p-4 md:mx-52 md:mt-0 2xl:mx-auto 2xl:w-7/12 2xl:p-16 ${
    count === 3 ? 'hidden' : 'z-0'
  }`;

  const skillsTitleAnimationClasses = `${count === 0 ? 'animate__delay-1s' : ''}`;

  return (
    <main
      className="relavite mb-20 flex h-screen min-w-280 flex-col items-center justify-center text-black dark:text-white md:mx-0"
      id="skills">
      {/* Estilos dinámicos para h1 */}
      <h1 className={skillsTitleClasses} onClick={handleZIndex}>
      ...
      <section className={skillsSectionClasses}>
        <div className={skillsContentClasses}>
          <div
            ...
            <h3
              className={`animate__animated animate__bounceInLeft relative mb-1 font-Kanit text-2xl dark:text-white 2xl:mb-4 2xl:text-4xl ${skillsTitleAnimationClasses}`}>
              Tengo conocimientos en:
            </h3>
            <p
              className={`animate__animated animate__bounceInLeft relative dark:text-white ${skillsTitleAnimationClasses}`}>
